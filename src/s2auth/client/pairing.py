...
class PairingClient:
    ...
    """
    def __init__(self, pairing_uri: str, pairing_token: str, storage: Dao, role: str, deployment: str, supported_s2_message_versions: List[str], supported_communication_protocols: List[str], supportedHmacHashingAlgorithms: List[str]) -> None:
        ...
        self._pairing_token_bytes: bytes = urlsafe_b64decode(pairing_token) if pairing_token else urlsafe_b64encode(token_bytes(32))
        self.pairing_token_str: str = pairing_token if pairing_token else urlsafe_b64encode(self._pairing_token_bytes).decode('UTF-8')
    ...
    async def pair(self, s2_client_description: S2NodeDescription, pairing_s2_node_id: Optional[str] = None) -> bool:
        ...
        body: dict[str, Any] = request_payload.model_dump(exclude_none=True)
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if self._role == S2Role.RM:
                    ...
                    await self.post_connection_details(pairing_response.pairingAttemptId.model_dump(exclude_none=True), connection_details, pairing_response.serverHmacChallenge)
    ...
    def sign_base64_with_hmac(self, b64_value: str) -> bytes:
        ...
        return b64encode(urlsafe_b64encode(mac))
    ...
    async def finalize_pairing(self, attempt_id: str, success: Optional[bool] = None) -> httpx.Response:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(f'{self._pairing_uri}/finalizePairing',
                                         headers=add_header(pairing_attempt_id=attempt_id),
                                         json=finalize_pairing_postRequest.model_dump(exclude_none=True))
            return response


class ConnectionClient:
    def __init__(self, client_uri: str, storage: Dao, role: str, deployment: str, supported_s2_message_versions: List[str], supported_communication_protocols: List[str]) -> None:
